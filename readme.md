...
ThreadBox is a threading library for JavaScript that is built upon NodeJS `worker_threads`. It is written to allow for compute heavy JavaScript and WASM to be trivially distributed to run across many threads by wrapping heavy calls within a class. ThreadBox can take any class and move it to a worker thread. It provides to the caller an asynchronous interface to the class to invoke its methods across thread boundaries, and provides clean worker thread disposal.
